...
## 21. Evaluation Notes for Graph Context Systems

This fixture is designed to test whether a graph/context extraction system can identify:
...
- Use structured queries instead of raw SQL.
- Keep the tool surface focused.
- Use SQLite plus filesystem artifacts.
- Use deterministic read-only failure modes.
- Avoid dedicated period-comparison tool.
- Use session-based AI workflow with saved Markdown outputs.
- Perform final polish in phases rather than rewriting everything.

### Important Dependencies

- Tool contracts depend on domain model, schema, filter semantics, and error model.
- Workflow mapping depends on stable tool contracts.
- Seed data depends on schema and workflow requirements.
- Testing depends on tools, workflows, seed data, and failure modes.
- Final spec assembly depends on all design artifacts.
- Final polish depends on review findings.

### Important Risks

- Overloaded filter terminology.
- Ambiguous `limit` semantics.
- Incomplete render/export requirements.
- Schema/tool mismatches.
- Tool surface creep.
- Testing strategy that does not actually validate workflows.
- Spec reading like brainstorming instead of a buildable contract.

### Expected Graph Clusters

A good context graph should likely form clusters around:
...
5. **Tool surface**
6. **Shared query/filter semantics**
7. **Workflow mapping**
8. **Seed data**
9. **Testing strategy**
10. **Review findings**
11. **Final polish plan**

---

## 22. Condensed Outcome

The conversation evolves from a broad request for assessment help into a structured, multi-session design workflow for producing a high-quality MCP server technical specification.

The main lesson is that a complex AI-assisted deliverable should be decomposed into reusable design artifacts, each with explicit dependencies, and then assembled into a final document only after the domain model, schema, tool surface, workflows, and testing strategy have stabilized.
